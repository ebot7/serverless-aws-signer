...
class ServerlessPlugin {
  ...
  async verifyConfiguration(configuration) {
    ...
    const profileArn = await signersMethods.getProfileParamByName(configuration.signerConfiguration.profileName, configuration.signerConfiguration.profileOwner, 'profileVersionArn', this.serverless)
  ...
  async addSigningConfigurationToCloudFormation() {
    ...
    for (let lambda in signerProcesses) {
      ...
      const profileOwner = signerProcesses[lambda].signerConfiguration.profileOwner;
      ...
      var profileArn = await signersMethods.getProfileParamByName(profileName, profileOwner, 'profileVersionArn', this.serverless);
  ...
  async removeResources() {
    ...
    // We are going to use cross-account signing and it cannot be removed
    // for (let lambda in signerProcesses) {
    //   var signItem = signerProcesses[lambda];
    //   if (!signItem.signerConfiguration.retain) {
    //     await this.removeSigningProfile(signItem.signerConfiguration.profileName, signItem.signerConfiguration.profileOwner)
    //   }
    // }
  }
  
  async removeSigningProfile(profileName, profileOwner) {

    // Make sure signing profile exists and hasn't been revoked yet
    try {

    const profileVersion = await signersMethods.getProfileParamByName(profileName, profileOwner, 'profileVersion', this.serverless)
